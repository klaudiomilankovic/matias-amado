...
import CrisalixReview from "./CrisalixReviews";

const Description = ({ id }) => {
  const { data, loading } = useFetch(`/trabajos/${id}`) as Props;

  // Only add CrisalixReview if the page is certain ID make an array of IDs
  const ids = ['129','124', '125', '126', '127', '128', '129', '130'];
  const showCrisalixReview = ids.includes(id);

  console.log(showCrisalixReview);
  ...
  return (
    <>
      ...
      {showCrisalixReview && <CrisalixReview />}
    </>
  );
};
